  ...
  <div class="app">
    <header>
      <div style="display:flex; align-items:center; gap:10px">
        <h1>🧭 Website Migration Planner</h1>
        <span class="badge" id="duplicateBadge" title="Duplicate URLs detected" style="display:none">⚠️ Duplicates</span>
      </div>
      <div class="controls">
        <button class="btn primary" id="addItemBtn">➕ Add Item</button>
        <button class="btn" id="bulkAddBtn">📥 Bulk Add</button>
        <button class="btn" id="importBtn">📂 Import JSON</button>
        <button class="btn" id="exportJsonBtn">⬇️ Export JSON</button>
        <button class="btn" id="exportCsvBtn">⬇️ Export CSV</button>
        <button class="btn ghost" id="clearBtn" title="Clear all items">🗑 Clear</button>
      </div>
    </header>
    ...
    <aside class="sidebar">
      ...
      <details class="tips" open>
        ...
        <ul class="tips-list">
          ...
          <li>Keep <strong>titles (≤60 chars)</strong> & meta descriptions (≤155 chars) unique.</li>
    ...
    <section class="main">
      <div class="panel">
        <div class="toolbar">
          <input type="search" id="search" placeholder="Filter by URL, title, owner, notes…" />
        ...
        <div id="emptyState" class="empty">No items yet. Add an item or use <span class="pill">📥 Bulk Add</span> to paste URLs.</div>
      </div>
    </section>
  </div>
  ...
  <div class="modal" id="itemModal" aria-hidden="true" role="dialog">
    <div class="box">
      <div style="display:flex; align-items:center; justify-content:space-between; gap:10px; margin-bottom:6px">
        ...
        <button class="btn" id="closeItemModal">✖</button>
      </div>
      <form id="itemForm">
        <div class="grid">
          <div class="col-12">
          ...
          <div class="col-8" style="display:flex; align-items:end; justify-content:flex-end; gap:8px">
            <button class="btn" type="button" id="suggestMetaBtn" title="Suggest basic metadata">✨ Suggest</button>
  ...
  <div class="modal" id="bulkModal" aria-hidden="true" role="dialog">
    <div class="box">
      <div style="display:flex; align-items:center; justify-content:space-between; gap:10px; margin-bottom:6px">
        ...
        <button class="btn" id="closeBulkModal">✖</button>
      </div>
      <div class="grid">
        <div class="col-12">
...
/images/hero.jpg"></textarea>
        ...
        <div class="col-12" style="display:flex; gap:8px; justify-content:flex-end">
          <button class="btn" id="bulkDetect">🔎 Preview</button>
  ...
  <script>
    ...
    function render(){
      ...
      filtered.forEach((it, idx)=>{
        ...
        const tr = $('<tr />');
        tr.append(`<td class="nowrap drag" title="Drag handle">☰ ${(i+1)}</td>`);
        ...
        actions.append($('<button class="btn" title="Edit">✏️</button>').on('click',()=>openEdit(i)));
        actions.append($('<button class="btn danger" title="Delete">🗑</button>').on('click',()=>del(i)));
        tr.append(actions);
        tbody.append(tr);
      });
