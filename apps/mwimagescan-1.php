<?php
// mwimagescan.php (Enhanced version with lazy loading detection)
...
// Now includes detection of lazy loading images.
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    foreach ($urls as $pageUrl) {
        ...
        foreach ($imgNodes as $imgNode) {
            // Check for various src attributes (including lazy loading)
            $src = trim($imgNode->getAttribute('src'));
            $dataSrc = trim($imgNode->getAttribute('data-src'));
            $dataLazySrc = trim($imgNode->getAttribute('data-lazy-src'));
            $dataSrcset = trim($imgNode->getAttribute('data-srcset'));
            $dataOriginal = trim($imgNode->getAttribute('data-original'));
            $dataUrl = trim($imgNode->getAttribute('data-url'));
            ...
            if (!empty($dataSrc)) {
                $actualSrc = $dataSrc;
                $lazyLoadingType = 'data-src';
            } elseif (!empty($dataLazySrc)) {
                $actualSrc = $dataLazySrc;
                $lazyLoadingType = 'data-lazy-src';
            } elseif (!empty($dataOriginal)) {
                $actualSrc = $dataOriginal;
                $lazyLoadingType = 'data-original';
            } elseif (!empty($dataUrl)) {
                $actualSrc = $dataUrl;
                $lazyLoadingType = 'data-url';
            } elseif (!empty($src)) {
                $actualSrc = $src;
                // Check if it has native lazy loading
                $loading = trim($imgNode->getAttribute('loading'));
                if (strtolower($loading) === 'lazy') {
                    $lazyLoadingType = 'loading="lazy"';
                }
            }
            ...
            if ($isInPicture) {
                ...
                foreach ($sourceNodes as $sourceNode) {
                    ...
                    $dataSrcset = trim($sourceNode->getAttribute('data-srcset'));
                    $media = trim($sourceNode->getAttribute('media'));
                    $type = trim($sourceNode->getAttribute('type'));
                    
                    $sourceSrc = !empty($dataSrcset) ? $dataSrcset : $srcset;
                    
                    if ($sourceSrc) {
                        $sourceInfo = $sourceSrc;
                        if ($media) $sourceInfo .= ' (' . $media . ')';
                        if ($type) $sourceInfo .= ' [' . $type . ']';
                        if (!empty($dataSrcset)) $sourceInfo .= ' (lazy)';
                        $sources[] = $sourceInfo;
                    }
                }
                if (!empty($sources)) {
            ...
            if ($actualSrc !== '') {
                ...
                if (strpos($actualSrc, 'data:') === 0) {
                    continue;
                }
                
                // Convert relative URLs to absolute
                if (strpos($actualSrc, 'http') !== 0) {
                    $parsedUrl = parse_url($pageUrl);
                    $baseUrl = $parsedUrl['scheme'] . '://' . $parsedUrl['host'];
                    if (substr($actualSrc, 0, 1) === '/') {
                        $actualSrc = $baseUrl . $actualSrc;
                    } else {
                        $actualSrc = $baseUrl . '/' . ltrim($actualSrc, '/');
                    }
                }
                
                // Get file extension and size info
                $pathInfo = pathinfo(parse_url($actualSrc, PHP_URL_PATH));
                ...
                if (!isset($imageData[$imageKey])) {
                    $imageData[$imageKey] = [
                        ...
                        'height' => $height,
                        'loading' => $loading,
                        'lazy_loading_type' => $lazyLoadingType,
                        'is_lazy_loaded' => !empty($lazyLoadingType),
        ...
        foreach ($imageData as $data) {
            $results[] = [
                ...
                'lazy_loading_type' => $data['lazy_loading_type'],
                'is_lazy_loaded' => $data['is_lazy_loaded'],
    ...
    foreach ($results as $row) {
        ...
        $ext = $row['extension'];
        $count = $row['count'];
        $isLazy = $row['is_lazy_loaded'];
        $lazyType = $row['lazy_loading_type'];
        ...
        if (!isset($summary[$src])) {
            $summary[$src] = [
                ...
                'extension' => $ext,
                'alt' => $row['alt'],
                'is_lazy_loaded' => $isLazy,
                'lazy_loading_type' => $lazyType,
                'total_count' => 0,
                'pages' => []
            ];
        }
        ...
        // Summary by lazy loading type
        if ($isLazy) {
            $totalLazyImages += $count;
            if (!isset($lazyLoadingSummary[$lazyType])) {
                $lazyLoadingSummary[$lazyType] = 0;
            }
            $lazyLoadingSummary[$lazyType] += $count;
        }
    ...
    echo json_encode([
        ...
        'lazyLoadingSummary' => $lazyLoadingSummary,
        'totalImages' => $totalImages,
        'totalLazyImages' => $totalLazyImages,
...
<body>
    <div class="container">
        <div class="header">
            ...
            <p class="subtitle">Discover all images used across your site (including lazy loaded images)</p>
        </div>

        <div class="card">
            <h2><i class="fas fa-info-circle"></i> About This Tool</h2>
            <p>This scanner analyzes your site's sitemap to identify all images being used across your pages. It finds <code>&lt;img&gt;</code> tags (including those inside <code>&lt;picture&gt;</code> elements), excluding base64 encoded images, and now <strong>detects lazy loading images</strong> using common attributes like <code>data-src</code>, <code>data-lazy-src</code>, <code>data-original</code>, and native <code>loading="lazy"</code>.</p>
        </div>
        ...
        <div class="card instructions">
            ...
            <ol>
                ...
                <li>Review detailed results showing every <code>&lt;img&gt;</code> tag found (including lazy loaded and those in <code>&lt;picture&gt;</code> elements)</li>
                <li>Check summary statistics for image usage patterns, file type distribution, and lazy loading usage</li>
            </ol>
        </div>
        ...
        <div class="results-container" id="results-container">
            ...
            <div class="card summary-section" id="summary-section">
                ...
                <h3 style="color: white; margin: 2rem 0 1rem 0;"><i class="fas fa-tachometer-alt"></i> Lazy Loading Usage</h3>
                <div class="table-wrapper">
                    <table id="lazy-loading-table" class="summary-table">
                        <thead>
                            <tr>
                                <th>Lazy Loading Type</th>
                                <th>Count</th>
                                <th>Percentage</th>
                            </tr>
                        </thead>
    ...
    $('#scan-form').on('submit', function(e) {
        ...
        $.post('<?php echo $_SERVER["PHP_SELF"]; ?>', { sitemap: url })
         .done(function(data) {
            ...
            var totalLazyImages = data.totalLazyImages || 0;
            ...
            $('#status').removeClass('loading error').addClass('success').text('Scan completed! Found ' + totalImages + ' images (' + totalLazyImages + ' lazy loaded) across ' + totalPages + ' pages.');
            ...
            rows.forEach(function(r) {
                ...
                var lazyLoadingInfo = '';
                if (r.is_lazy_loaded) {
                    var lazyType = r.lazy_loading_type || 'lazy';
                    lazyLoadingInfo = '<span class="lazy-badge">LAZY</span><br><small style="color: var(--neutral-gray); font-size: 0.75rem;">' + lazyType + '</small>';
                } else {
                    lazyLoadingInfo = '<span style="color: var(--neutral-gray); font-size: 0.85rem;">Standard</span>';
                }
            ...
            // Create stats cards
            var lazyPercentage = totalImages > 0 ? ((totalLazyImages / totalImages) * 100).toFixed(1) : '0';
            ...
            $statsGrid.html(
                ...
                '<div class="stat-card"><span class="stat-number">' + totalLazyImages + '</span><div class="stat-label">Lazy Loaded (' + lazyPercentage + '%)</div></div>' +
                '<div class="stat-card"><span class="stat-number">' + totalPages + '</span><div class="stat-label">Pages Scanned</div></div>'
            );

            // Populate lazy loading usage table
            var lazyData = data.lazyLoadingSummary || {};
            var $lazyTbody = $('#lazy-loading-table tbody');
            $lazyTbody.empty();
            
            // Add "Not Lazy Loaded" entry
            var standardImages = totalImages - totalLazyImages;
            if (standardImages > 0) {
                var standardPercentage = ((standardImages / totalImages) * 100).toFixed(1);
                $lazyTbody.append(
                    $('<tr>').append(
                        $('<td>').text('Standard Loading'),
                        $('<td>').html('<span class="count-badge" style="background: var(--neutral-gray);">' + standardImages + '</span>'),
                        $('<td>').text(standardPercentage + '%')
                    )
                );
            }
            
            // Sort lazy loading types by count (descending)
            var sortedLazyTypes = Object.keys(lazyData).sort((a, b) => lazyData[b] - lazyData[a]);
            
            sortedLazyTypes.forEach(function(lazyType) {
                var count = lazyData[lazyType];
                var percentage = ((count / totalImages) * 100).toFixed(1);
                $lazyTbody.append(
                    $('<tr>').append(
                        $('<td>').html('<span class="lazy-badge">' + lazyType.toUpperCase() + '</span>'),
            ...
            sortedImages.forEach(function(src) {
                ...
                var lazyInfo = '';
                if (imageData.is_lazy_loaded) {
                    var lazyType = imageData.lazy_loading_type || 'lazy';
                    lazyInfo = '<span class="lazy-badge">LAZY</span><br><small style="color: var(--neutral-gray); font-size: 0.75rem;">' + lazyType + '</small>';
                } else {
                    lazyInfo = '<span style="color: var(--neutral-gray); font-size: 0.85rem;">Standard</span>';
                }
