...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $maxPages = max(1, min($maxPages, 100)); // Limit between 1-100 pages
...
<body>
    <div class="container">
        ...
        <div class="card instructions">
            ...
            <ol>
                ...
                <li>Choose how many pages to analyze (1-100 pages, default: 10)</li>
    ...
    <script>
    let auditResults = []; // Store results for CSV export
    ...
    $('#export-csv-btn').on('click', function() {
        if (!auditResults || auditResults.length === 0) {
            alert('No audit results to export. Please run an audit first.');
            return;
        }
        ...
        auditResults.forEach(function(result) {
            var row = [
                ...
                result.requests || 0,
                result.images ? result.images.total : 0,
                result.images ? result.images.without_alt : 0,
                result.images ? result.images.without_lazy : 0,
                result.images ? result.images.large_images : 0,
                result.css_files || 0,
                result.js_files || 0,
                result.inline_css || 0,
                result.inline_js || 0,
                result.async_js || 0,
                result.defer_js || 0,
                result.has_viewport ? 'Yes' : 'No',
                result.has_gzip ? 'Yes' : 'No',
        ...
        link.setAttribute("download", "performance_audit_" + timestamp + ".csv");
